...
@Component({
  ...
  template: `
        ...
        <label>Big list example</label>
        <ng-select [items]="bigList"
                   bindLabel="title"
                   placeholder="Select photo"
                   [(ngModel)]="selected_photo">
        </ng-select>
        <p>
            Selected photo: {{selected_photo | json}}
        </p>
        <hr />
...
export class AppComponent {
  ...
  private create10kCities() {
    this.getBigList().subscribe((data) => {
      this.bigList = data;
    });
  }
  getBigList(): Observable<any> {
    console.log('hahaha');
